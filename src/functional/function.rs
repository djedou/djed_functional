...
use std::rc::Rc;
use djed::djed::{
    AnyScope,Component, ComponentLink, Html, Props
};
...
where
    T: FunctionProvider,
{
    type State = Box<dyn FnOnce() -> bool>;
    type Props = T::TProps;
    ...
    fn update(&mut self, msg: Self::State) -> bool {
        msg()
    }

    fn change(&mut self, props: Self::Props) -> bool {
        let mut props = props;
...
impl<T: 'static> FunctionComponent<T>
where
    T: FunctionProvider,
{
